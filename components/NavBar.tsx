...
const WA = "https://wa.me/4915906171828?text=" + encodeURIComponent("Hello VisaVista! I need help with my European visa.");
...
const destinations = [
  ...
  { href: "/italy",  flag: "🇮🇹", name: "Italy",          sub: "New — Decreto Flussi 2026", live: true },
  { href: "/sweden", flag: "🇸🇪", name: "Sweden",         sub: "Work & study permits", live: true },
  ...
  return (
    <>
      ...
      <div style={{ background:"#0B1D3A", padding:"8px 0", textAlign:"center", fontSize:"12.5px", color:"rgba(255,255,255,.65)" }}>
        ...
        <a href={WA} target="_blank" rel="noopener noreferrer" style={{color:"#25D366",fontWeight:700,textDecoration:"underline",textUnderlineOffset:"2px"}}>
          💬 WhatsApp us →
        </a>
      </div>
      ...
      <header style={{ position:"sticky", top:0, zIndex:100, background:"rgba(255,255,255,.98)", backdropFilter:"blur(20px)", WebkitBackdropFilter:"blur(20px)", borderBottom:"1px solid #E5E7EB", boxShadow:"0 2px 16px rgba(0,0,0,.06)" }}>
        <div style={{ maxWidth:"1200px", margin:"0 auto", padding:"0 1.5rem", display:"flex", alignItems:"center", justifyContent:"space-between", height:"68px", gap:"12px" }}>
          ...
          <Link href="/" style={{ display:"flex", alignItems:"center", gap:"11px", textDecoration:"none", flexShrink:0 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:"9.5px", fontWeight:600, color:"#9CA3AF", letterSpacing:"0.08em", textTransform:"uppercase" }}>DE · IT · SE · UK</div>
            </div>
          </Link>

          {/* Desktop nav */}
          <nav style={{ display:"flex", alignItems:"center", gap:"2px", flex:1, justifyContent:"center" }} className="vv-desktop-nav">
            <Link href="/" style={{ fontSize:"13.5px", fontWeight:600, color: pathname==="/" ? "#DC2626":"#374151", padding:"8px 10px", borderRadius:"10px", textDecoration:"none", background: pathname==="/" ? "#FEF2F2":"transparent" }}>Home</Link>
            ...
            <div ref={dropRef} style={{ position:"relative" }}>
              <button onClick={() => setDestOpen(!destOpen)} style={{ fontFamily:"Inter,ui-sans-serif", fontSize:"13.5px", fontWeight:600, color: destOpen?"#DC2626":"#374151", padding:"8px 10px", borderRadius:"10px", background: destOpen?"#FEF2F2":"none", border:"none", cursor:"pointer", display:"flex", alignItems:"center", gap:"4px" }}>
                Destinations
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ transform: destOpen?"rotate(180deg)":"none", transition:"transform .2s" }}>
                  <path d="M6 9l6 6 6-6"/>
                </svg>
              </button>
              {destOpen && (
                <div style={{ position:"absolute", top:"calc(100% + 8px)", left:0, background:"#fff", borderRadius:"18px", border:"1px solid #E5E7EB", boxShadow:"0 20px 56px rgba(0,0,0,.12)", padding:"8px", minWidth:"240px", zIndex:200 }}>
                  {destinations.map(c => (
            ...
            {[
              {href:"/work",l:"Work"},{href:"/study",l:"Study"},{href:"/finances",l:"💳 Banking"},
              {href:"/appointment",l:"Appointments"},{href:"/service",l:"Services"},
            ].map(n => (
              <Link key={n.href} href={n.href} style={{ fontSize:"13.5px", fontWeight:600, color: pathname===n.href ? "#DC2626":"#374151", padding:"8px 10px", borderRadius:"10px", textDecoration:"none", background: pathname===n.href ? "#FEF2F2":"transparent" }}>{n.l}</Link>
            ))}
          </nav>

          {/* Right */}
          <div style={{ display:"flex", alignItems:"center", gap:"8px", flexShrink:0 }}>
            <a href={WA} target="_blank" rel="noopener noreferrer" className="vv-desktop-nav" style={{ display:"inline-flex", alignItems:"center", gap:"6px", padding:"9px 16px", borderRadius:"10px", background:"#25D366", color:"#fff", textDecoration:"none", fontSize:"13px", fontWeight:700 }}>
              💬 WhatsApp
            </a>
            <Link href="/consultancy" className="vv-desktop-nav" style={{ display:"inline-flex", alignItems:"center", gap:"6px", padding:"9px 18px", borderRadius:"10px", background:"#DC2626", color:"#fff", textDecoration:"none", fontSize:"13px", fontWeight:700, boxShadow:"0 4px 12px rgba(220,38,38,.3)" }}>
              📅 Free Call
            </Link>
        ...
        {mobileOpen && (
          <div style={{ borderTop:"1px solid #E5E7EB", background:"#fff", padding:"12px 16px 20px" }}>
            {links.map(l => (
              <Link key={l.href} href={l.href} onClick={() => setMobileOpen(false)} style={{ display:"block", padding:"11px 14px", borderRadius:"12px", fontSize:"14px", fontWeight: pathname===l.href ? 700 : 500, color: pathname===l.href ? "#DC2626":"#111827", background: pathname===l.href ? "#FEF2F2":"transparent", textDecoration:"none", marginBottom:"2px" }}>{l.label}</Link>
            ))}
