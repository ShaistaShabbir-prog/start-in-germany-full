    ...
    <div style={{background:"#0a0d14",minHeight:"100vh",color:"#f1f5f9",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      <div style={{background:"linear-gradient(135deg,#111520,#1a1f2e)",borderBottom:"1px solid rgba(255,255,255,.07)",padding:"32px 24px"}}>
        <div style={{maxWidth:"960px",margin:"0 auto"}}>
          ...
          <h1 style={{fontSize:"2rem",fontWeight:900,marginBottom:"8px"}}>Work &amp; Live in Netherlands 🇳🇱</h1>
        ...
        <div style={{maxWidth:"960px",margin:"0 auto",display:"flex",overflowX:"auto"}}>
          {TABS.map(t => (
            <button key={t} onClick={()=>setTab(t)} style={{padding:"12px 20px",border:"none",background:"transparent",
              color:tab===t?"#FF6B35":"#64748b",fontWeight:700,fontSize:".85rem",
              borderBottom:`2px solid ${tab===t?"#FF6B35":"transparent"}`,cursor:"pointer",whiteSpace:"nowrap"}}>
      ...
      <div style={{maxWidth:"960px",margin:"0 auto",padding:"24px 16px"}}>
        {tab==="overview" && (
          <div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:"12px",marginBottom:"20px"}}>
              {[["Currency","EUR"],["Immigration","International"],["Authority","IND (Immigration NL)"],["Visa options","4+"]].map(([l,v]) => (
                <div key={l} style={{background:"#111520",border:"1px solid rgba(255,255,255,.07)",borderRadius:"12px",padding:"16px",textAlign:"center"}}>
                  <div style={{fontSize:"1.1rem",fontWeight:800,color:"#FF6B35",marginBottom:"4px"}}>{v}</div>
            ...
            <div style={{background:"#111520",border:"1px solid rgba(255,255,255,.07)",borderRadius:"14px",padding:"20px"}}>
              <h2 style={{fontWeight:700,marginBottom:"10px"}}>Why Netherlands?</h2>
        ...
        {tab==="visa" && (
          <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
            <div style={{background:"#111520",border:"1px solid rgba(255,255,255,.07)",borderRadius:"14px",padding:"16px",borderLeft:"4px solid #FF6B35"}}>
              <div style={{fontWeight:700,color:"#FF6B35",marginBottom:"4px"}}>Highly Skilled Migrant</div>
              <div style={{fontSize:".8rem",color:"#94a3b8",lineHeight:"1.6"}}>Contact IND (Immigration NL) for requirements, thresholds, and processing times. Requirements vary by nationality.</div>
            </div>
            <div style={{background:"#111520",border:"1px solid rgba(255,255,255,.07)",borderRadius:"14px",padding:"16px",borderLeft:"4px solid #FF6B35"}}>
              <div style={{fontWeight:700,color:"#FF6B35",marginBottom:"4px"}}>ICT Permit</div>
              <div style={{fontSize:".8rem",color:"#94a3b8",lineHeight:"1.6"}}>Contact IND (Immigration NL) for requirements, thresholds, and processing times. Requirements vary by nationality.</div>
            </div>
            <div style={{background:"#111520",border:"1px solid rgba(255,255,255,.07)",borderRadius:"14px",padding:"16px",borderLeft:"4px solid #FF6B35"}}>
              <div style={{fontWeight:700,color:"#FF6B35",marginBottom:"4px"}}>Startup Visa</div>
              <div style={{fontSize:".8rem",color:"#94a3b8",lineHeight:"1.6"}}>Contact IND (Immigration NL) for requirements, thresholds, and processing times. Requirements vary by nationality.</div>
            </div>
            <div style={{background:"#111520",border:"1px solid rgba(255,255,255,.07)",borderRadius:"14px",padding:"16px",borderLeft:"4px solid #FF6B35"}}>
              <div style={{fontWeight:700,color:"#FF6B35",marginBottom:"4px"}}>Orientation Year Visa</div>
              <div style={{fontSize:".8rem",color:"#94a3b8",lineHeight:"1.6"}}>Contact IND (Immigration NL) for requirements, thresholds, and processing times. Requirements vary by nationality.</div>
            </div>
          </div>
        )}
        {tab==="salary" && (
          <div style={{background:"#111520",border:"1px solid rgba(255,255,255,.07)",borderRadius:"14px",padding:"20px"}}>
            <h2 style={{fontWeight:700,marginBottom:"14px"}}>Salary Guide (EUR)</h2>
              ...
              <tbody>
                {[["Software Engineer","€90,000"],["Data Scientist","€85,000"],["Product Manager","€95,000"],["Healthcare Professional","€70,000"],["Financial Analyst","€80,000"]].map(([r,s])=>(
                  <tr key={r} style={{borderBottom:"1px solid rgba(255,255,255,.07)"}}>
                    ...
                    <td style={{padding:"9px 12px",color:"#FF6B35",fontWeight:700}}>{""+s}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {tab==="steps" && (
          <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
            {["Research visa type and eligibility",`Gather documents — degree, employment contract, CV`,`Apply online via IND (Immigration NL)`,
              ...
              <div key={i} style={{display:"flex",gap:"10px",alignItems:"flex-start",background:"#111520",border:"1px solid rgba(255,255,255,.07)",borderRadius:"12px",padding:"13px"}}>
                <div style={{width:"26px",height:"26px",borderRadius:"50%",background:"#FF6B35",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:".78rem",flexShrink:0}}>{""+( i+1)}</div>
        ...
        {tab==="links" && (
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:"10px"}}>
            {[{n:"IND (Immigration NL)",d:"Official immigration authority",u:"https://www.google.com/search?q=IND+(Immigration+NL)+immigration"},
              ...
              <a key={l.n} href={l.u} target="_blank" rel="noopener"
                ...
                <div style={{fontWeight:700,color:"#FF6B35",marginBottom:"4px"}}>{""+l.n}</div>
...
{/* ── VISIT VISA SECTION ── */}
<section style={{background:"#FFFBF0",borderTop:"3px solid #AE1C28",padding:"4rem 0"}}>
  <div style={{...sec}}>
    <span style={{...eye,color:"#AE1C28"}}>🇳🇱 Visit / Tourist Visa</span>
    <h2 style={{fontFamily:PD,fontSize:"clamp(1.6rem,3vw,2.3rem)",fontWeight:800,color:"#0B1D3A",lineHeight:1.2,marginBottom:"8px"}}>Netherlands / Schengen Tourist Visa (Type C)</h2>
    <div style={{...bar,background:"#AE1C28"}}/>
    <p style={{fontSize:"15px",color:"#6B7280",marginBottom:"2rem",maxWidth:"640px"}}>Apply at Dutch Embassy if Netherlands is your primary destination</p>
    ...
    <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:"2rem",alignItems:"start"}}>
      <div style={{background:"#fff",border:"1.5px solid #E5E7EB",borderRadius:"16px",padding:"1.5rem"}}>
        ...
        <ul style={{padding:0,margin:0}}>
              ...
              <CI key="4">Return flight and accommodation proof</CI>
              <CI key="5">Bank statements (3 months)</CI>
              <CI key="6">Employment letter + NOC</CI>
              <CI key="7">Invitation letter if visiting Dutch host</CI>
        </ul>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:"1rem"}}>
        <div style={{background:"#FEF3C7",border:"1.5px solid #FCD34D",borderRadius:"16px",padding:"1.5rem"}}>
          ...
          <p style={{fontSize:"13px",color:"#78350F",lineHeight:1.6,margin:0}}>TLS Contact handles visa appointments in Pakistan for Netherlands. Apply 3+ months before travel as appointment slots fill up quickly.</p>
        </div>
        <a href="https://www.netherlandsandyou.nl/your-country-and-the-netherlands/pakistan/visas-and-travel" target="_blank" rel="noopener noreferrer"
          style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",padding:"14px 24px",
            borderRadius:"12px",background:"#AE1C28",color:"#fff",fontWeight:700,fontSize:"15px",
